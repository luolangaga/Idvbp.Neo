...
public sealed class WebBackendWindow : Window
{
    private readonly NativeWebView _webView;
    private readonly string _url;

    public WebBackendWindow(string title, string url)
    {
        _url = url;
        Title = title;
        Width = 1280;
    ...
    private Control BuildLayout(string title, string url)
    {
        ...
        header.Children.Add(switchButton);
        header.Children.Add(closeButton);
        Grid.SetColumn(urlText, 1);
        Grid.SetColumn(reloadButton, 2);
        Grid.SetColumn(switchButton, 3);
    ...
    public void NavigateTo(string url)
    {
        try
        {
            _webView.Source = new Uri(url, UriKind.Absolute);
        }
        catch
        {
        }
