   ...
   */
		build( data, geometry /*, onProgress, onError */ ) {
      	 ...
      	 */
				...
				// 强制改用 MeshPhongMaterial，这是目前最稳定的方案，能彻底解决 Shader 编译错误
				const phongMat = new THREE.MeshPhongMaterial( params );
				phongMat.skinning = true;
				
				// 如果有 gradientMap (toon 贴图)，尝试应用到 map 上或者忽略
				// MMD 的 toon 贴图在标准材质中很难直接对应，但为了稳定显示，先保住模型
		...
		_loadTexture( filePath, textures, params, onProgress, onError ) {
			...
			if ( params.isDefaultToonTexture === true ) {
				...
				// 修复：处理反斜杠并进行路径编码，确保中文贴图文件名能正确加载
				const normalizedPath = filePath.replace( /\\/g, '/' );
				// 只有当 resourcePath 是 file:// 协议时才进行额外编码处理
				if ( this.resourcePath.startsWith( 'file://' ) ) {
					const pathParts = normalizedPath.split( '/' );
					const encodedParts = pathParts.map( function ( part ) {

						return encodeURIComponent( part );

					} );
					fullPath = this.resourcePath + encodedParts.join( '/' );
				} else {
					fullPath = this.resourcePath + normalizedPath;
				}
			...
			// 增加调试日志
			console.log( '[MMDLoader] Requesting texture:', fullPath );
			...
			const texture = loader.load( fullPath, function ( t ) {
				...
				console.log( '[MMDLoader] Texture loaded successfully:', fullPath );
				...
				console.error( '[MMDLoader] Texture load failed:', fullPath, err );
			...
			for ( const propertyName of exposePropertyNames ) {
				...
				Object.defineProperty( this, propertyName, {
					...
					set: function ( value ) {
						...
						// 自动更新 defines
						if ( propertyName === 'map' ) {
							this.defines.USE_MAP = !!value;
							this.defines.USE_UV = !!value;
							this.needsUpdate = true;
						}
						if ( propertyName === 'matcap' ) {
							this.defines.USE_MATCAP = !!value;
							this.needsUpdate = true;
						}
						if ( propertyName === 'envMap' ) {
							this.defines.USE_ENVMAP = !!value;
							this.needsUpdate = true;
						}
						if ( propertyName === 'emissiveMap' ) {
							this.defines.USE_EMISSIVEMAP = !!value;
							this.needsUpdate = true;
						}
