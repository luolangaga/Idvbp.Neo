...
`canvas.scaleMode` 用于控制设计稿画布如何适配浏览器视口：

| 值 | 说明 |
| --- | --- |
| `contain` | 默认值，等比缩放并完整显示画布；视口比例不一致时会留空 |
| `cover` | 等比缩放并铺满视口；视口比例不一致时会裁切画布 |
| `stretch` | 非等比拉伸铺满视口；适合需要兼容多种输出比例的导播包 |
...
页面编辑模式下，组件可以在注册定义里提供 `contextMenu` 或 `editorContextMenu`，用于扩展编辑器右键菜单。菜单只在 `/bp-layout?...&edit=1` 中显示，不影响正式运行页面。

```js
window.IdvbpLayoutRuntime.register("my-background", {
    render(element, props, context) {
        const config = context.config || {};
        element.style.backgroundImage = config.image
            ? `url("${context.frontendBase}/${config.image}")`
            : "";
    },
    contextMenu({ config }) {
...
| --- | --- |
| `node` | 当前 layout node |
| `context` | 当前组件 render 使用的 context |
| `props` | 当前已解析的 props |
| `config` | 当前 node 的组件配置，已尝试 `JSON.parse` |
| `helpers` | 编辑器提供的辅助方法 |
...
| 方法 | 说明 |
| --- | --- |
| `helpers.setConfig(value)` | 保存当前组件实例配置，等价于写入 `frontend:{packageId}:{pageId}:component:{nodeId}` |
| `helpers.importAsset(file, category)` | 上传文件到当前前台包，返回 `{ fileName, relativePath, url, sizeBytes }` |
| `helpers.update()` | 重新渲染当前组件 |
| `helpers.close()` | 关闭右键菜单 |

支持的菜单项类型：

| type | 说明 |
| --- | --- |
| `file` | 文件选择，常用于导入图片、视频等资源 |
| `select` | 下拉选择，使用 `options: [{ value, label }]` |
| `text` | 文本输入 |
| `number` | 数字输入，可设置 `min`、`max`、`step` |
| `color` | 颜色选择 |
| `checkbox` | 布尔开关 |
| `button` | 命令按钮，使用 `action(value, helpers)` |
| `separator` | 分隔线 |

文件资源导入后会复制到：

```text
wwwroot/frontends/{packageId}/assets/{category}/
```

组件配置里建议保存 `asset.relativePath`，例如 `assets/backgrounds/bg.png`。组件渲染时再通过 `context.frontendBase` 或本地 `resolveAsset` 辅助函数解析为可访问 URL。这样导出前台包 ZIP 时，资源和配置会一起打包。
...
| `POST /api/frontends/{id}/assets/import` | 上传资源到当前前台包的 `assets/{category}` 目录 |
| `GET /api/frontends/fonts` | 列出 `wwwroot/font` 中可用字体 |
| `POST /api/frontends/fonts/import` | 上传字体到 `wwwroot/font` |

## 运行时日志 API

前端组件和 layout runtime 通过 `navigator.sendBeacon` 把日志异步发到后端，后端统一收集在内存中。前端不需要打开浏览器开发者工具即可查看运行时日志。

### 前端使用方式

layout runtime 和 designer 组件脚本已内置 `rtLog(source, level, ...args)` 函数：

```js
rtLog("designer", "info", "syncState fired eventType=", event.type);
rtLog("runtime", "error", "SignalR failed", error.message);
```
...
| 参数 | 说明 |
|------|------|
| `source` | 日志来源，如 `"designer"`、`"runtime"`、`"my-component"` |
| `level` | 日志级别：`"debug"`、`"info"`、`"warn"`、`"error"` |
| `...args` | 任意内容，对象会自动 `JSON.stringify` |

`rtLog` 会同时做两件事：
1. `console.log` 输出到浏览器控制台
2. 通过 `navigator.sendBeacon("/api/runtime-logs", ...)` 异步发送到后端

### 后端 API

| API | 说明 |
|------|------|
| `POST /api/runtime-logs` | 写日志。请求体 `{ source, level, message }` |
| `GET /api/runtime-logs?count=200` | 读取最近日志，默认 200 条 |
| `DELETE /api/runtime-logs` | 清空所有日志 |
...
```bash
curl http://localhost:5000/api/runtime-logs
curl -X DELETE http://localhost:5000/api/runtime-logs
```

### 在自定义组件中使用

如果需要在自定义组件脚本中写日志，可以直接调用 `navigator.sendBeacon`：

```js
function myLog(level, ...args) {
    const message = args.map(a => typeof a === "object" ? JSON.stringify(a) : String(a ?? "")).join(" ");
    console.log(...args);
    try { navigator.sendBeacon("/api/runtime-logs", JSON.stringify({ source: "my-component", level, message })); } catch {}
}

myLog("info", "render", context.node.id);
```

日志会同时输出到：
1. **后端控制台** — 每条日志实时打印
2. **日志文件** — 按天切分，保存在 `logs/runtime/runtime-{yyyyMMdd}.log`

日志格式示例：

```text
[14:32:18.421] [INFO] [designer] syncState fired eventType=room.snapshot rules=3
[14:32:19.105] [DEBUG] [runtime] dispatchLayoutEvent type=room.phase.updated nodes=12
```
